...
module Cache#(
    ...
    wire                        flush;
    reg  [INDEX_W-1  : 0]       counter, counter_nxt;

    // SRAM Memory.
    reg  [BIT_W*4-1 : 0]        data [0 : BLOCK_W-1]; // 1KB
    reg  [TAG_W+1 : 0]          tag [0 : BLOCK_W-1];
    ...
    assign	proc_index      = (data_byte_offset > proc_offset)?(i_proc_addr[7:4])-1:i_proc_addr[7:4];
    assign	proc_tag        = i_proc_addr[31:8];
    ...
    assign sram_cache_data = (cache_sram_cen) ? data[cache_sram_index] : 128'b0;
    assign sram_cache_tag  = (cache_sram_cen) ? tag[cache_sram_index] : 24'b0;
    ...
    always @(*) begin
        if (o_cache_available) begin
            done_nxt = (counter == BLOCK_W-1 && mem_stall == 0)?1'b1:1'b0;
        end
        else begin
            done_nxt = 1'b1;
        end
    ...
    always @(*) begin
        if (flush) begin
            counter_nxt = (mem_stall)?counter:counter+1;
        end
        else begin
            counter_nxt = 1'b0;
        end
    ...
    always @(*) begin
        // FLUSH
        flush_tag = tag[counter];
        flush_address = {flush_tag[TAG_W-1 : 0], counter, data_byte_offset};
        flush_data = data[counter];
        // FSM
        mem_cen = 0;
        mem_wen = 0;
        cache_wen = 0;
            ...
            case (state)
                ...
                S_ALLOC: begin
                    ...
                    if (mem_stall) begin
                        ...
                        // $display("i_mem_data: %h", i_mem_rdata);
                        mem_cen = 1'b0;
                        cache_wen = 1'b1;
                        state_nxt = S_UPDATE;
                    end
        ...
        if (!i_rst_n) begin
            ...
            if (cache_sram_cen) begin
                ...
                begin
                    ...
                    data[cache_sram_index] <= cache_sram_data;
                    tag[cache_sram_index] <= cache_sram_tag;
                end
            end
        end
